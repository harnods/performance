...
- See `components/AddGoalDrawer.vue`'s `memberBox`/`memberBoxRow`/`memberLoadMoreBar` for the reference implementation — reused as-is (same box, same row shape) for Goal contributor's Company/Individual list (owner-seeded, freely add/remove). Goal members paginates 10 at a time (`MEMBER_PAGE`); that contributor list also paginates 10 at a time (`CONTRIB_PAGE`, per-owner since a bulk-created goal has one independent list per owner) — page size isn't fixed by the pattern, pick what fits the list's typical size. Team/Org's contributor radios don't reuse this box at all: "All goal members" shows nothing below it (it's just the Goal members list above, restating it would be pure duplication), and "Selected goal members" is an `MpInputTag` picker instead (see [`form.md`](form.md)'s "Multi-select rendered as removable tags"), since removing a scoped-to-members pick doesn't need the full identity-row treatment and tags don't paginate.
