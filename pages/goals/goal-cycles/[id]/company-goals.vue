...
// (groupByOwner:false — company goals merge a category across owners), so the
// sort runs BEFORE withRowSpans and keeps those runs contiguous: a grouping-
// column sort reorders whole blocks; Goal/Owner/Progress/Status reorder only
// rows inside the innermost sub-category block. sortKey '' = default order.
const sortKey = ref('')
...
const rows = computed(() => withRowSpans(sortGoalRows(sortByCategory(filteredGoals.value, { groupByOwner: false }), {
